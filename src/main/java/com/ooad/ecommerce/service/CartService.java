...
public class CartService {
  ...
  public Map<Product,Integer> getCartDetails(Integer userId) {
    ...
    List<Product> productList = productRepository.findAllById(cartList.stream().map(entry -> entry.getId().getProductId()).collect(Collectors.toList()));
    Map<Integer, Product> productMap = productList.stream().collect(Collectors.toMap(Product::getId, product -> product));
    return cartList.stream().collect(Collectors.toMap(entry -> productMap.get(entry.getId().getProductId()), Cart::getQuantity));
  }
